...
<?php if(getAccess(get_current_user_id())->access == 1 || get_current_user_id() == $_GET['id']):?>
<?php $result = $wpdb->get_row($wpdb->prepare( "SELECT `user_id`, `surname`, `name`, `patronymic`, `iin`, `tel`, `email`, `access`, `date_create`, `user_id_attached` FROM p_user_fields WHERE user_id = %d", $_GET['id'])); ?>	
		...
		<div class="card">
			...
			<div class="card-body">
				<div class="row">
					...
					<div class="col-sm-4">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" required class="form-control" data-user  name="u_patronymic" value="<?= $result->patronymic ?>"><div class="form-control-line"></div>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input type="email" required class="form-control" data-user  name="u_email" value="<?= $result->email ?>"><div class="form-control-line"></div>
					</div>
				</div>
				<div class="form-group">
				...
				<?php if($result->access == '4' || $result->access == '1' || $result->access == '5'):?>
				<?php else: ?>
				<?php
				if($result->access === '2' ) {
					$results = $wpdb->get_results("SELECT u.user_id, u.surname, u.name, u.patronymic
							FROM p_user_fields u
							WHERE u.access in (3, 4);");
				}
					...
					?>
				<div class="form-group" <?php if(getAccess(get_current_user_id())->access != 1) echo "hidden"; ?>>
					<label for="password13" class="col-sm-2 control-label">Закрепленные пользователи</label>
					<div class="col-sm-10">
						<select class="form-control select2-list " name="user_attached" multiple="" data-attached id="attached" >
						<option></option>						
							<?php foreach ($results as $value):?>
								<?php foreach(explode(",", $result->user_id_attached) as $useratt):?>
									<?php if($useratt == $value->user_id): ?>
				...
				<label for="Password5" class="col-sm-2 control-label">Пароль</label>
					<div class="input-group">
						<div class="input-group-content">
							<input type="text" id="password" data-user  class="form-control" name="u_pass"><div class="form-control-line"></div>
						</div>
