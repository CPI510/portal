...
<?php else: ?>
    ...
    <?php $result = $wpdb->get_row($wpdb->prepare( "SELECT id, p_name, active FROM p_programs WHERE id = %d", $_GET['id'])); ?>	
    ...
    <?php else: ?>
        ...
        if($_POST['p_name'] && $_POST['active'] && $_POST['id']) {
            $wpdb->query($wpdb->prepare( $s="UPDATE p_programs SET p_name = %s, active = %s WHERE id = %d", $_POST['p_name'], $_POST['active'], $_POST['id']));
