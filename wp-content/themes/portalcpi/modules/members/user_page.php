...
<?php $result = $wpdb->get_row($wpdb->prepare( "SELECT u.user_id, u.surname, u.name, u.patronymic, u.iin, u.tel, u.email, u.access, u.date_create, a.name_ru access_name_ru, a.name_kz access_name_kz FROM p_user_fields u
            ...
            <div class="row style-primary">
                <div class="col-sm-10">
                    <div class="card-body style-default-dark">
                        ...
                        <div class="text-default-light"><?= $result->{"access_name_" . $_SESSION['lang']} ?></div>
                    </div>
                </div><!--end .col -->
                <div class="col-sm-2">
                    <div class="card-body">
                ...
                <div class="col-md-12">
                    <article class="style-default-bright">
                        ...
                        <div class="col-md-6">
                            ...
                            <?php if($result->access === '4' || $result->access === '3' || $result->access === '2' || $result->access === '1'): ?>
                                <h3 class="text-light"> Папки сотрудников портала</h3>
                                ...
                                <ul class="nav nav-pills nav-stacked nav-transparent">
                                    ...
                                    <?php foreach($dataFolders as $folder): ?>
                                        <?php if ($folder->access_id): ?>
                                            ...
                                            ?>
                                            <?php if ($arrAccessId[$result->access] == $result->access): ?>
                            ...
                            <?php endif; ?>
                                ...
                                <?php foreach($dataCourses as $data): ?>
                                    ...
                                    <?php $dataFolders = $wpdb->get_results($wpdb->prepare("SELECT f.id folder_id, f.name folder_name, p.user_id, COUNT(p.id) Num_f
                                        ...
                                        GROUP BY f.id
                                        ",$result->user_id, $data->id_group, $data->program_id ));?>
                                    <?php foreach($dataFolders as $folder): ?>
                                        <li><a href="/users/?z=add_file&folder_id=<?= $folder->folder_id ?>&user_id=<?= $result->user_id ?>&trener=1&group_id=<?= $data->id_group ?>"><span class="badge pull-right"><?= $folder->Num_f ?></span><?= $folder->folder_name ?></a></li>
                                    <?php endforeach; ?>
                                <?php endforeach; ?>
                                </ul>
                                

                            <?php if($result->access === '3' || $result->access === '2'|| $result->access === '1'): ?>
                            <?php $numU = nameUser($result->user_id,4);
                            $attachedU = $wpdb->get_results($wpdb->prepare("SELECT u.user_id, u.surname, u.name, u.patronymic, u.email, a.name_ru access_name_ru, a.name_kz access_name_kz
                            ...
                            INNER JOIN p_access a ON a.value = u.access WHERE user_id in ($numU)"));?>
                                ...
                                <?php foreach($attachedU as $user): ?>
                                    <li><a href="/members/?z=user_page&id=<?= $user->user_id ?>"> <?= $user->surname ?> <?= $user->name ?> <?= $user->patronymic ?> / <?= $user->{"access_name_" . $_SESSION['lang']} ?></a></li>
                                <?php endforeach; ?>
                                </ul>
                            <?php endif; ?>
                        </div><!--end .col-md-6 -->
...
<?php else:?>
<br><?php //alertStatus('warning', 'Нет доступа!', true); //redirectBack();
?>
<?php endif; ?>