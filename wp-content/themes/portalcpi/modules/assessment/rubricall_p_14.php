...
<?php if( $grinf->program_id == 14 && getAccess(get_current_user_id())->access == 1 ){
...
<?php $r=0; foreach ($data as $res): $r++; ?>
    ...
    if($grinf->trener_id == $res->create_user_id){
        ...
        $position = "Тимлидер";
        $download_tm2 = '<a href="/export_to_word/?form=assessment_p_14&create_user_id='.$res->create_user_id.'&listener_id='.$res->listener_id.'&group='.$_GET['id'].'&ver=2" class="btn btn-primary">'.  DOWNLOAD_JUSTIFICATION .'</a>';
...
//    }
    ...
    ${'tabdata_'.$r} = '<div role="tabpanel" class="tab-pane '.$active.'" id="tabdata_'.$r.'"><div class="card">
        <a href="/export_to_word/?form=assessment_p_14&create_user_id='.$res->create_user_id.'&listener_id='.$res->listener_id.'&group='.$_GET['id'].'" class="btn btn-primary">' . ASSESSMENT_SECOND[0] . '</a>
        '.$download_tm2.'
       '.$rubric_file_text.'
	<div class="card-body">
		<div class="panel-group" id="accordion1">
			<div class="card panel">
				<div class="card-head collapsed" data-toggle="collapse" data-parent="#accordion1" data-target="#accordion1-1'.$r.'" aria-expanded="false">
					<header>' . ASSESSMENT_RUBRIC_14[0] . '</header>
						<div class="tools">
		...
		<div class="panel-group" id="accordion1">
			<div class="card panel">
				<div class="card-head collapsed" data-toggle="collapse" data-parent="#accordion1" data-target="#accordion1-2'.$r.'" aria-expanded="false">
					<header>' . ASSESSMENT_RUBRIC_14[2] . '</header>
						<div class="tools">
							<a class="btn btn-icon-toggle"><i class="fa fa-angle-down"></i></a>
						</div>
				</div>
		...
		<div class="panel-group" id="accordion1">
			<div class="card panel">
				<div class="card-head collapsed" data-toggle="collapse" data-parent="#accordion1" data-target="#accordion1-3'.$r.'" aria-expanded="false">
					<header>' . ASSESSMENT_RUBRIC_14[4] . '</header>
					<div class="tools">
						<a class="btn btn-icon-toggle"><i class="fa fa-angle-down"></i></a>
					</div>
				</div>
