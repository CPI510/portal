...
if($_GET['tm'] == 'all'){
    ...
    $tamplateName = 'templates/'.$groupInfo->program_id.'/template_rubric_'.$lang_t.'_teamleader_all.docx';
...
//        $templateProcessor->setValue('rowNumber#'.$t, htmlspecialchars($t));
        ...
        $templateProcessor->setValue('section_d_grade#'.$t, $arrGrade[$res->section_d_grade]);
        $templateProcessor->setValue('section_e_grade#'.$t, $arrGrade[$res->section_e_grade]);
    ...
    $rubric_text = 'Обоснование_';
    if ($res->create_user_id == $groupInfo->moderator_id){
        $position = "модератора";
        $position2 = "Модератор";
        $tamplateName = 'templates/'.$groupInfo->program_id.'/template_justification_'.$lang_t.'_moderator.docx';
    } elseif ($res->create_user_id == $groupInfo->expert_id){
        $tamplateName = 'templates/'.$groupInfo->program_id.'/template_justification_'.$lang_t.'_expert.docx';
    }
...
//$_doc = new \PhpOffice\PhpWord\TemplateProcessor('Template.docx');
    ...
    $templateProcessor->setValue('section_d_grade', $arrGrade[$res->section_d_grade]);
    $templateProcessor->setValue('section_e_grade', $arrGrade[$res->section_e_grade]);
