...
function login_header( $title = 'Log In', $message = '', $wp_error = null ) {
	...
	<title><?php echo $login_title; ?></title>
        <link type="text/css" rel="stylesheet" href="/wp-content/themes/portalcpi/assets/css/theme-1/bootstrap.css?1422792965" />
        <link type="text/css" rel="stylesheet" href="/wp-content/themes/portalcpi/assets/css/theme-1/materialadmin.css?1425466319" />
        <link type="text/css" rel="stylesheet" href="/wp-content/themes/portalcpi/assets/css/theme-1/font-awesome.min.css?1422529194" />
        <link type="text/css" rel="stylesheet" href="/wp-content/themes/portalcpi/assets/css/theme-1/material-design-iconic-font.min.css?1421434286" />
        <link type="text/css" rel="stylesheet" href="/wp-content/themes/portalcpi/assets/css/theme-1/libs/select2/select2.css?1424887856" />
        <link type="text/css" rel="stylesheet" href="/wp-content/themes/portalcpi/assets/css/theme-1/libs/bootstrap-datepicker/datepicker3.css?1424887858" />
	<?php
    ...
    if(isset($_GET['reboot']) && $_GET['reboot'] == 1) {
        $url = str_replace("&reboot=1","",$_SERVER['REQUEST_URI']);
	...
	<div id="login">
        ...
        <ul class="header-nav header-nav-options">
            <li>
                <button type="submit" class="btn btn-icon-toggle ink-reaction <?= ($_SESSION['lang'] == 'kz') ? "active": "" ?>"><i class="fa "><a href="<?php

                        if(isset($_GET['lang']) && $_GET['action'] != 'lostpassword') {
                            echo substr($_SERVER['REQUEST_URI'], 0, -8) . "?lang=kz&reboot=1";
                        }elseif($_GET['action'] == 'lostpassword' && !isset($_GET['lang'])){
                            echo $_SERVER['REQUEST_URI'] . "&lang=kz&reboot=1";
                        }elseif($_GET['action'] == 'lostpassword' && isset($_GET['lang'])){
                            echo $_SERVER['HTTP_REFERER'] . "&lang=kz&reboot=1";
                        }elseif($_GET['veryficationerror'] == '1'){
                            echo $_SERVER['REQUEST_URI'] . "&lang=kz&reboot=1";
                        }else{
                            echo $_SERVER['REQUEST_URI'] . "?lang=kz&reboot=1";
                        }
                        ?>">KZ</a></i></button>
                <button type="submit" class="btn btn-icon-toggle ink-reaction <?= ($_SESSION['lang'] == 'ru') ? "active": "" ?>"><i class="fa "><a href="<?php
                        if(isset($_GET['lang']) && $_GET['action'] != 'lostpassword') {
                            echo substr($_SERVER['REQUEST_URI'], 0, -8) . "?lang=ru";
                        }elseif($_GET['action'] == 'lostpassword' && !isset($_GET['lang'])){
                            echo $_SERVER['REQUEST_URI'] . "&lang=ru";
                        }elseif($_GET['action'] == 'lostpassword' && isset($_GET['lang'])){
                            echo $_SERVER['HTTP_REFERER'] . "&lang=ru";
                        }elseif($_GET['veryficationerror'] == '1'){
                            echo $_SERVER['REQUEST_URI'] . "&lang=ru";
                        }else{
                            echo $_SERVER['REQUEST_URI'] . "?lang=ru";
...
switch ( $action ) {
	...
	case 'retrievepassword':
		...
		//login_header( __( 'Lost Password' ), '<p class="message">Өзіңіздің Email адресіңізді енгізіңіз. Сіз парольді қалпына келтіру бойынша нұсқаулықтары бар email хатты аласыз <br> Введите ваш адрес email. Вы получите email сообщение с инструкциями по сбросу пароля.</p>', $errors );
		...
		<form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( network_site_url( 'wp-login.php?action=lostpassword', 'login_post' ) ); ?>" method="post">
			<p>
				<label for="user_login">Email<?php //_e( 'Username or Email Address' ); ?></label>
		...
		if ( 'confirm' === $_GET['checkemail'] ) {
			$errors->add(
				...
				sprintf(
					...
					//'Растау сілтемесі жіберілетін поштаңызды тексеріңіз, содан кейін <a href="%s">кіру бетіне</a> кіріңіз. <br> Проверьте вашу почту для ссылки с подтверждением, затем зайдите <a href="/login/">на страницу входа</a>.',
	...
	default:
		...
		<form name="loginform" id="loginform" action="<?php echo esc_url( site_url( 'wp-login.php', 'login_post' ) ); ?>" method="post">
            <?= ( $_GET['veryficationerror'] == 1 && isset($_GET['id']) ) ? '<div id="login_error"><h2>'.VERIFICATION_EMAIL[0].'</h2> 
<br>'.VERIFICATION_EMAIL[1].' <a href="/login/?veryficationerror=2&id='.$_GET['id'].'" class="btn btn-success">'.VERIFICATION_EMAIL[2].'</a></div>' : null ?><p>
           <p>
            <?php if( $_GET['veryficationerror'] == 2 && isset($_GET['id']) ){
                mailVerificationSend($_GET['id']);
             echo '<p class="message">На Ваш почтовый ящик отправлено сообщение, содержащее ссылку для подтверждения правильности e-mail адреса. Пожалуйста, перейдите по ссылке для завершения регистрации.
Если письмо не пришло в течение 15 минут, проверьте папку «Спам».</p>';
            } ?><p>
